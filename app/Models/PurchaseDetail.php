	...
	protected static function booted()
	{
		...
		static::created(function ($purchaseDetails) {
			...
			if ($product) {
				...
				$product->stock += $purchaseDetails->quantity;
				$product->save();
			}
		});
		...
		static::updated(function ($purchaseDetails) {
			$product = $purchaseDetails->product;
			if ($product) {
				// Obtener la cantidad anterior antes de la actualización
				$originalQuantity = $purchaseDetails->getOriginal('quantity');
				$newQuantity = $purchaseDetails->quantity;
